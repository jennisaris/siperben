...
class T_terbit_sk extends MX_Controller {
	...
	function create_sk_from_template($datas=null, $template='sk.docx') {
		...
		$keempat = "Dengan berlakunya keputusan ini, Keputusan Menteri Pendidikan, Kebudayaan, Riset, dan Teknologi {$no_sk2} tentang Pejabat Perbendaharaan pada {$unit_kerja}, dicabut dan dinyatakan tidak berlaku.";
    ...
    function save() {
		...
		*/
		...
		$index = 0;
		foreach($this->input->post('check_pegawai_status') as $k=>$v){
			...
			if ( $v == 1 ) {
				...
				$iusetetap = $this->input->post('check_pegawai_iusetetap')[$k];
		...
		//echo 'SK : '.$no_sk.' => '.$tgl_sk.' => '.$tmt_sk.' => '.$sk_id.' => '.$iusetetap;
		//echo 'LAINNYA : '.$nama_unor.' => '.$id_unor.' => '.$sk_id;
		//exit;
		...
		/* $SQLP = "SELECT a.*, 
		 ...
		 $SQLP = "SELECT a.*,
				...
				from kepeg_m_golongan where id = a.cgolid) as pangkat_nama,
				(select nama from kepeg_m_jabatan where kode = a.cjabid) as jabatan_nama,
				...
				group by a.cnip  
				order by a.ijabid2 asc";//and a.itipe = 0
		...
		foreach($rowsP as $r) {
			...
			$datas = ['norut' => $r->ijabid2, 'jenis_jabatan' => $r->jabatan2_nama, 
		...
		$datas2['datanya'] = [
								...
								'tgl_bulan_tahun2' => ($iusetetap == 1 ? $this->nama_bulan_indonesia($tgl_tetap) : ''),
								'unit_kerja'=>ucwords(strtolower($nama_unor)),
								'unit_kerja_upper'=>strtoupper($nama_unor),
								'isplt'=>$ttd->isplt,
								'nama_pejabat_ttd'=>$nama_pejabat_ttd,
	...
	function lists($page_ke=0) {
		...
		$sql = "SELECT app_t_usulan.ijns as app_t_usulan_ijns, app_t_usulan.id as app_t_usulan_id, app_t_usulan.iunorid as app_t_usulan_iunorid, 
				...
				case 
				  when app_t_usulan.itipe = 0 then app_t_usulan.istatus in (4,6) and app_t_usulan.ctahun = '{$this->session->settahun}' 
	...
	function generate_no_sk_sinde($params=array()) {
		...
		$result = $this->rest->post('services_pkln/getToken', 
			/* array(
				'client-id'=>'52b5a3e2-a6bf-4f0c-b238-744a4e8769d8', 
				'client-secret'=>'c94e80a5-d088-4ccc-9f3a-82640a75d8bf'), false
		); */
		array(
