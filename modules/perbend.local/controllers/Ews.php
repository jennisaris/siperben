  ...
  function lists($page_ke=0, $reports=false) {
    ...
    $today = date('Y-m-d');
		/*$sql = "SELECT kepeg_m_pegawai.id, kepeg_m_pegawai.cnip, kepeg_m_pegawai.vname, 
	  	...
	  	kepeg_m_pegawai.cnobnt,
		  kepeg_m_pegawai.cnopnt,
		  kepeg_m_pegawai.cnosnt
				from kepeg_m_pegawai where 
				((cnobnt IS NOT NULL AND 
(Right(cnobnt, 4) + 5 = year(curdate()))) OR 
				(cnopnt IS NOT NULL AND 
(Right(cnopnt, 4) + 5 = year(curdate()))) OR
				(cnosnt IS NOT NULL AND 
(Right(cnosnt, 4) + 5 = year(curdate()))))";*/
    $sql = "SELECT kepeg_m_pegawai.id, kepeg_m_pegawai.cnip,kepeg_m_pegawai.vname, 
          	  	kepeg_m_pegawai.cjabid2,
    ...
    if ( $this->session->superuser != 1 ) {
      ...
      exit;
		...
		$pagination = $this->_ajaxPagination(base_url()."perbend/ews/lists", $this->kriteria, 'ews');
