  ...
  var $parent_unorid;
	...
	function updateBox_app_t_usulan_sk_dtgltetap($name, $value, $datas) {
		...
		$input .= "<div style='margin-top:-35px;margin-left:110px;'><input {$chk_iusetetap} type='checkbox' name='{$name}_chk' id='{$name}_chk' onchange='chg_iusetetap(this);'/>
					<div style='margin-top:-30px!important;margin-left:17px;'><b>Gunakan Sbg TMT Ditetapkan</b></div>
					</div>";
	...
	function insertBox_app_t_usulan_sk_cnosk($name) {
		$value = '';
		$input = "<input readonly placeholder='Generated from SINDE' value='".$value."' type='text' name='{$name}' 
			id='{$name}' class='form-control {$name}'/>";

		return $input;
	}
	
	function updateBox_app_t_usulan_sk_nosk($name, $value, $datas) {
		$value = ($value == '' ? '' : $value);
		$input = "<input readonly placeholder='Generated from SINDE' value='".$value."' type='text' name='{$name}' 
			id='{$name}' class='form-control {$name}'/>";
