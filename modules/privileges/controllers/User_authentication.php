...
class User_authentication extends MX_Controller {
	...
	public function doLogin()
	{
		...
		$this->form_validation->set_rules('captcha', 'Kode Captcha', 'trim|required');

		if ($this->form_validation->run() === FALSE) {
			$data = $this->set_captch(); // load ulang captcha
			$this->form_validation->set_error_delimiters(
				'<div style="text-align:left;margin-top:5px;" class="alert alert-danger" role="alert">',
				'</div>'
			);
			$this->template->display('user/form_login', $data);
			return;
		...
		if (!$this->is_login_allowed()) {
			$data = $this->set_captch();
			$data['error_message'] = '<div class="alert alert-danger">Terlalu banyak percobaan login dari IP Anda. Silakan coba lagi dalam 15 menit.</div>';
			$this->template->display('user/form_login', $data);
			return;
		}
	...
	function set_captch() {
        ...
        'img_path'      => 'uploads/captcha/',
        'img_url'       => base_url().'uploads/captcha/',
        'font_path'     => FCPATH . 'fonts/Open_Sans/open-sans.Regular.ttf', // Pastikan jalur font valid
        'img_width'     => '180',
        'img_height'    => 50,
        'expiration'    => 7200,
        'word_length'   => 5,
        'font_size'     => 20, // Coba gunakan ukuran font yang lebih besar untuk melihat perubahan
        'img_id'        => 'Imageid',
        'pool'          => '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ',
