...
class User_authentication extends MX_Controller {
	...
	public function doLogout() {			
		...
		$this->session->unset_userdata('ar_menu');
			...
			if ( $query->num_rows() > 0 ) {
				...
				if ( password_verify($password, $hash) ) {
				    ...
				    if ( $this->theme == 'sb-admin-2' ) {
					    $sess_data['ar_menu'] = $this->rekursifSBAdmin2(0, $sess_data['groupid'], 1);
				    } else if ( $this->theme =='AdminLTE-2.3.11' ) {
					    $sess_data['ar_menu'] = $this->rekursifAdminLTE(0, $sess_data['groupid'], 1);	
				    } else {
					    $sess_data['ar_menu'] = $this->rekursifMenu(0, $sess_data['groupid'], 1);
				    }
