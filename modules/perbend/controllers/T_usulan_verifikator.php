...
class T_usulan_verifikator extends MX_Controller {
  var $prefix = 'app';
  ...
  var $ar_statusperubahan = array();
	...
	public function __construct() {
		parent::__construct();
		$controller = "perbend/t_usulan_verifikator";
		...
		$this->_addQuery($table, 'app_t_usulan.istatus = 1', 'and', '', 'true');
	...
	function manipulate_list_button($buttons) {
			$buttons['add'] = "<button type='button' onclick='save_verifikasi1(\"".base_url()."perbend/t_usulan_verifikator\", \"t_usulan_verifikator\")' class='btn btn-primary' name='save_checklist' id='save_checklist' class='form-control save_checklist'>
	...
	public function manipulate_update_button($buttons, $datas) {
	  $btn_simpankirim = "<button type='submit' id='btn_send' class='btn btn-primary' style='{$style}' 
								onclick='tinyMCE.triggerSave(true,true);$(\"#t_usulan_verifikator_form-edit #app_t_usulan_istatus\").val(1);'>
	...
	function app_t_usulan_output(){
        $js ="<script type='text/javascript'>
                  ...
                  function klik(dis, b) {
					...
					if ( b == 1 ) {
						...
						if ( $('.check_usulan1').eq(i).is(':checked') ) {
							$('.check_usulan1_status').eq(i).val(1);
						...
						if ( $('.check_usulan2').eq(i).is(':checked') ) {
							$('.check_usulan2_status').eq(i).val(1);
          ...
          function save_verifikasi1(url, table_id, default_txt_confirm='Simpan. Anda yakin?', _ismodal=false, _modals='form-modal', _islochref=false, _isneedrefresh=true, _isOldFashion=false, _msg='Simpan Persetujuan berhasil.') {
                    ...
                    save_confirm(url+'/save_verifikasi1', formData, default_txt_confirm, table_id, _ismodal, function(output) {
      			...
      			if ( !empty($check_usulan1_status[$k]) ) {
					...
					$post = [
      				...
      				if ( $check_usulan_jenis[$k] == 2 )	{
						...
						$status += 3;
						$query2[] = "update app_t_usulan_pegawai set istatus = 1
										where iusulanid = {$v} and istatus = 0";
			...
			if ( !empty($check_usulan2_status[$k]) )  { 		
				...
				if ( $check_usulan_jenis[$k] == 2 )	{
					$query2[] = "update app_t_usulan_pegawai 
							set istatus = 2, valasan='".$alasan_usulan2[$k]."' 
							where iusulanid = {$v}";
				} else {
