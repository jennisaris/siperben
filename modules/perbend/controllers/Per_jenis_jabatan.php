...
class Per_jenis_jabatan extends MX_Controller {
  var $prefix = 'app';
  var $table;
	public function __construct() {
		parent::__construct();
		$controller = "perbend/per_jenis_jabatan";
		$this->table  = $this->prefix."_t_usulan";

    //$this->_setModal(true);
    $this->_setTitle('Laporan Bendahara Bersertifikat Per  Jenis Jabatan');
    ...
    $detail .= "</tr>";
      ...
      foreach($cols as $c) { 
        ...
        $detail .= "<td style='cursor:pointer;text-align:center;'>".$c->tot_ada."</td>"; 
        $detail .= "<td style='cursor:pointer;text-align:center;'>".$c->tot_tdk_ada."</td>"; 
        $detail .= "</tr>";
	  ...
	  $pagination = '';//$this->_ajaxPagination(base_url()."perbend/per_jenis_jabatan/lists", $this->kriteria, 't_terbit_sk');
  		if ($reports) {
  ...
  function manipulate_list_button($buttons) {
    ...
    $input = "<div class='modal fade' id='myModal_browse' role='dialog' aria-labelledby='myModalLabel' data-backdrop='static' data-keyboard='false'>
		...
		$buttons['download'] = "<button class='btn btn-primary' type='button' name='btn_download' id='btn_download' onclick='download(\"".base_url()."perbend/per_jenis_jabatan/lists/0/1\");'><i class='fas fa-download'></i> Download</button>";
