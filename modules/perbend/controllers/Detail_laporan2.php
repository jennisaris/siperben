...
class Detail_laporan2 extends MX_Controller {
  var $prefix = 'app';
  var $table;
  ...
  var $kriteria = [];
	public function __construct() {
		parent::__construct();
		$controller = "perbend/detail_laporan2";
		$this->table  = $this->prefix."_t_usulan";

    $this->_setTitle('Laporan PPSPM Bersertifikat (Per Satuan Kerja) ');
      ...
      $html .= "<th>Jumlah</th>";
  		...
  		foreach($rows as $kode) {
  		  $total_all = 0;
				...
				$orgs =[trim($kode->kode)];
				$kode_lama = explode(',',$this->getrow('', 'priv_t_user', 'kode_lama', ['username'=>trim($kode->kode)])->kode_lama);
        ...
        $orgs = $orgsx;
  		  ...
  		  $tot_all_4 += $totals['total_4'];
  		  $tot_all_all += $tot_all_4;
        ...
        */
  		...
  		$html .= "</table>";
  		$pagination = $this->_ajaxPagination(base_url()."perbend/detail_laporan2/lists", $this->kriteria, 'detail_laporan2');
  		if ($reports) {
    ...
    $detail .= "<th>Sudah Memiliki SNT</th>";
    $detail .= "<th>Belum Memiliki SNT</th>";
    $detail .= "</tr>";
      ...
      foreach($cols as $c) { 
        ...
        $detail .= "<td style='cursor:pointer;text-align:center;' data-toggle='modal' data-target='#myModal_browse' onclick='_browse(\"".base_url()."perbend/detail_info2/index/0\");
        $(\"#detail_info2 #q_app_t_usulan_pegawai_cnosertifikat\").val(1);
        $(\"#detail_info2 #q_app_t_usulan_pegawai_ckduker\").val(\"{$kode}\");
        $(\"#detail_info2 #q_app_t_usulan_pegawai_ijabid2\").val(\"{$c->id}\");
        '>".$c->tot_ada."</td>"; 
        $detail .= "<td style='cursor:pointer;text-align:center;' data-toggle='modal' data-target='#myModal_browse' onclick='_browse(\"".base_url()."perbend/detail_info2/index/0\");
        $(\"#detail_info2 #q_app_t_usulan_pegawai_cnosertifikat\").val(0);
        $(\"#detail_info2 #q_app_t_usulan_pegawai_ckduker\").val(\"{$kode}\");
        $(\"#detail_info2 #q_app_t_usulan_pegawai_ijabid2\").val(\"{$c->id}\");
        '>".$c->tot_tdk_ada."</td>"; 
        $detail .= "</tr>";
        ...
        $detail .= "</tr>";
	  /*if (!$reports) {
  	  $btn_download = "<button class='btn btn-primary' type='button' name='btn_download_detail[]' id='btn_download_detail_{$unor_id}' onclick='download(\"".base_url()."perbend/detail_laporan2/_detail/{$kode}/{$orgs}/1\");'><i class='fas fa-download'></i> Download</button>";
  	  $detail .= "<tr>";
  ...
  function manipulate_list_button($buttons) {
    ...
    $input = "<div class='modal fade' id='myModal_browse' role='dialog' aria-labelledby='myModalLabel' data-backdrop='static' data-keyboard='false'>
		...
		$buttons['download'] = "<button class='btn btn-primary' type='button' name='btn_download' id='btn_download' onclick='download(\"".base_url()."perbend/detail_laporan2/lists/0/1/\"+$(\"#q_app_t_usulan_iunorid\").val());'><i class='fas fa-download'></i> Download</button>";
