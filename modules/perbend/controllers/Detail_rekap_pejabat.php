...
class Detail_rekap_pejabat extends MX_Controller {
  var $prefix = 'app';
  var $table;
  ...
  var $kriteria = [];
	public function __construct() {
		parent::__construct();
		$controller = "perbend/detail_rekap_pejabat";
	...
	function lists($page_ke=0, $reports=false, $kodeatasan='') {
  		...
  		$html .= "<th>Eselon I</th>";
  		$html .= "<th>Kode</th>";
  		$html .= "<th>Nama Satuan Kerja</th>";
  		
  		$sql = "SELECT ckode FROM app_m_jabatan order by id asc";
      ...
      foreach($cols as $c) {
        ...
        $orgs = $orgsx;
  		  ...
  		  $html .= "<tr>";
  		  $html .= "<td>".$norut."</td>";
  		  $html .= "<td>".$kode->eselon1."</td>";
  		  $html .= "<td>".$kode->kode."</td>";
  		  $html .= "<td>".$kode->nama."</td>";
        ...
        */
  		...
  		$html .= "</table>";
  		$pagination = $this->_ajaxPagination(base_url()."perbend/detail_rekap_pejabat/lists", $this->kriteria, 'detail_rekap_pejabat');
  		if ($reports) {
    ...
    $detail .= "</tr>";
      ...
      foreach($cols as $c) { 
        ...
        $detail .= "</tr>";
	  /*if (!$reports) {
  	  $btn_download = "<button class='btn btn-primary' type='button' name='btn_download_detail[]' id='btn_download_detail_{$unor_id}' onclick='download(\"".base_url()."perbend/detail_rekap_pejabat/_detail/{$kode}/{$orgs}/1\");'><i class='fas fa-download'></i> Download</button>";
  	  $detail .= "<tr>";
  ...
  function manipulate_list_button($buttons) {
    ...
    $input = "<div class='modal fade' id='myModal_browse' role='dialog' aria-labelledby='myModalLabel' data-backdrop='static' data-keyboard='false'>
		...
		$buttons['download'] = "<button class='btn btn-primary' type='button' name='btn_download' id='btn_download' onclick='download(\"".base_url()."perbend/detail_rekap_pejabat/lists/0/1/\"+$(\"#q_app_t_usulan_iunorid\").val());'><i class='fas fa-download'></i> Download</button>";
