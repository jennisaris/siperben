...
class T_usulan_satker extends MX_Controller {
  ...
  var $ar_unor = array();
	public function __construct() {
		...
		$this->_addField($table, 'daftar', '', false, false, true);
		...
		$this->_add2ListField($table, 'iunorid, cnousul, dtglusul, istatusid, ijnsprubhnid, lampiran, istatus, keterangan');//, tupdated, cupdatedby');
		...
		*/
		...
		if ( !$this->session->isadmin ) {			
			//$this->_addQuery($table, $table.".iunorid = '".trim($this->session->username)."'", 'and', '', true);
			$this->username2 = implode("','", $this->session->username2);
			$this->_addQuery($table, $table.".iunorid in ('".trim($this->username2)."')", 'and', '', true);
		} else {
		  ...
		  //exit;
			...
			$this->_add2SearchField($table, 'iunorid', true, false, false);

			//print_r($_POST);exit;
			if ( $this->input->post('q_app_t_usulan_iunorid') != '' ) {
				$kode_lama = explode(',',$this->getrow('', 'priv_t_user', 'kode_lama', ['username'=>$this->input->post('q_app_t_usulan_iunorid')])->kode_lama);
				if ( sizeOf($kode_lama) > 0 ) {
					$kode_lama_ = "'".implode("','", $kode_lama)."'";
					$this->_addQuery($table, $table.".iunorid in ({$kode_lama_})", "and", "", true);
				} else $this->_addQuery($table, $table.".iunorid = '".$this->input->post('q_app_t_usulan_iunorid')."' ", "and", "", true);
			}
	...
	function updateBox_app_t_usulan_daftar($name, $value, $datas, $params) {
		...
		$sql = "SELECT id, cnip, vname, vjabnm, 
				(select vname from app_m_jabatan where id = ijabid2) as jabatan_perbendaharan
				FROM `app_t_usulan_pegawai`
				WHERE `ckduker` = '{$datas->app_t_usulan_iunorid}' 
				AND `isnonaktif` = '0' AND `itipe` = '1'";
		$result = $this->db->query($sql)->result();
		$no = 1;
		foreach($result as $r) {
	...
	function viewBox_app_t_usulan_daftar($name, $value, $datas, $params) {
  	...
  	function searchBox_app_t_usulan_iunorid($name) {
		$select  = "<select name='{$name}' class='form-control {$name}' id='{$name}' onchange='reload_grid(\"".base_url()."perbend/t_usulan_satker/lists\", \"t_usulan_satker\");'>";
		$select .= "<option value=''>ALL</option>";
		foreach($this->getall('', 'app_m_unor', 'kode, nama', ['deleted'=>0]) as $r) {
			$select .= "<option value='{$r->kode}'>{$r->nama}</option>";
		}
		$select .= "</select>";
	...
	public function after_insert_processor($id, $post) {
		...
		if ( ( 
			...
			/* $sql = "INSERT INTO app_t_usulan_pegawai 
					...
					cnipold, itolak) 
					SELECT ifrom, {$id} as iusulanid, ispelatihan, cnip, vname, 
					case 
					 when (ijabid2 = 2 or ijabid2=3 or ijabid2 =6) then (select cnobnt from kepeg_m_pegawai where cnip = app_t_usulan_pegawai.cnip) 
					 when ijabid2 = 4 then (select cnosnt from kepeg_m_pegawai where cnip= app_t_usulan_pegawai.cnip) 
					 when ijabid2 = 5 then (select cnopnt from kepeg_m_pegawai where cnip=app_t_usulan_pegawai) 
					end as cnosertifikat, cgolid, 
					vgolnm, vpktnm, ijabid, vjabnm, ijabid2, istatus, valasan, 
					istatus2, valasan2, cnosk, inoskid, ckduker, 
					tcreated, ccreatedby, tupdated, cupdatedby, itipe, isnonaktif, 
			...
			if ( !$this->session->isadmin ) {			
				$kode_lama_ = "'".implode("','", $this->session->username2)."'";
			} else {
				$kode_lama = explode(',',$this->getrow('', 'priv_t_user', 'kode_lama', ['username'=>$post->app_t_usulan_iunorid])->kode_lama);
				if ( sizeOf($kode_lama) > 0 ) 
					$kode_lama_ = "'".implode("','", $kode_lama)."'";
				else $kode_lama_ = $post->app_t_usulan_iunorid;
			}
			...
			/* $sql = "INSERT INTO app_t_usulan_pegawai 
					...
					tcreated, ccreatedby, tupdated, cupdatedby, itipe, isnonaktif, 
					cnipold, itolak) 
					SELECT 0, {$id} as iusulanid, 0, cnip, vname, 
					case 
					 when (cjabid2 = 2 or cjabid2=3 or cjabid2 =6) then cnobnt 
					 when cjabid2 = 4 then cnosnt 
					 when cjabid2 = 5 then cnopnt 
					end as cnosertifikat, cgolid, 
					(select nama from kepeg_m_golongan where id = cgolid) as vgolnm, 
					(select pangkat from kepeg_m_golongan where id = cgolid) as vpktnm, 
					ijabid, 
					(select nama from kepeg_m_jabatan where id = ijabid) as vjabnm, 
					cjabid2, 1 as istatus, '' as valasan, 
					1 as istatus2, '' as valasan2, 
					(select cnosk from app_t_usulan_sk where iunorid in ({$kode_lama_})  
					order by id desc limit 1) as cnosk, 
					(select id from app_t_usulan_sk where iunorid in ({$kode_lama_}) 
					order by id desc limit 1) as inoskid, '{$post->app_t_usulan_iunorid}' as ckduker, 
					now() as tcreated, '{$this->session->username}' as ccreatedby, 
					now() as tupdated, '{$this->session->username}' as cupdatedby, 
					1 as itipe, 0 as isnonaktif, 
					NULL as cnipold, 0 as itolak 
					FROM kepeg_m_pegawai WHERE 
					(select kode_satker from kepeg_m_unor where id = kepeg_m_pegawai.ikduker) in ({$kode_lama_}) AND cjabid2 IN (1,2,3) 
					group by cjabid2"; */
			...
			$sql = "INSERT INTO app_t_usulan_pegawai 
					...
					tcreated, ccreatedby, tupdated, cupdatedby, itipe, isnonaktif, 
					cnipold, itolak) 
					SELECT 0, {$id} as iusulanid, 0, a.cnip, a.vname, 
					case 
					 when (a.cjabid2 = 2 or a.cjabid2=3 or a.cjabid2 =6) then cnobnt 
					 when a.cjabid2 = 4 then cnosnt 
					 when a.cjabid2 = 5 then cnopnt 
					end as cnosertifikat, a.cgolid, 
					(select nama from kepeg_m_golongan where id = a.cgolid) as vgolnm, 
					(select pangkat from kepeg_m_golongan where id = a.cgolid) as vpktnm, 
					a.ijabid, 
					(select nama from kepeg_m_jabatan where id = a.ijabid) as vjabnm, 
					b.ijabid2, 1 as istatus, '' as valasan, 
					1 as istatus2, '' as valasan2, 
					(select cnosk from app_t_usulan_sk where iunorid in ({$kode_lama_})  
					order by id desc limit 1) as cnosk, 
					(select id from app_t_usulan_sk where iunorid in ({$kode_lama_}) 
					...
					1 as itipe, 0 as isnonaktif, 
					NULL as cnipold, 0 as itolak 
					FROM kepeg_m_pegawai a, app_t_usulan_pegawai b WHERE 
					a.cnip = b.cnip and b.isnonaktif = 0 
					and b.ckduker in ({$kode_lama_}) AND b.ijabid2 IN (1,2,3) 
					group by b.ijabid2";
			...
			/* $sql = "INSERT INTO app_t_usulan_pegawai 
					...
					cjabid2, 1 as istatus, '' as valasan, 
					1 as istatus2, '' as valasan2, 
					(select cnosk from app_t_usulan_sk where iunorid = '{$post->app_t_usulan_iunorid}' 
					order by id desc limit 1) as cnosk, 
					(select id from app_t_usulan_sk where iunorid = '{$post->app_t_usulan_iunorid}' 
					order by id desc limit 1) as inoskid, '{$post->app_t_usulan_iunorid}' as ckduker, 
					now() as tcreated, '{$this->session->username}' as ccreatedby, 
					now() as tupdated, '{$this->session->username}' as cupdatedby, 
					1 as itipe, 0 as isnonaktif, 
			...
			$sql = "UPDATE app_t_usulan_pegawai SET 
					...
					WHERE ckduker in ({$kode_lama_}) AND ijabid2 IN (1,2,3) AND isnonaktif = '0' 
					and iusulanid != {$id} ";
	...
	function listBox_ACTION($buttons, $datas) {
		...
		if ( $datas->app_t_usulan_istatus != 5) {
			...
			$kode_lama = explode(',',$this->getrow('', 'priv_t_user', 'kode_lama', ['username'=>trim($this->session->username)])->kode_lama);	
			if ( $total > 0 || 
				//trim($datas->app_t_usulan_ccreatedby) != trim($this->session->username)
				/* ( trim($datas->app_t_usulan_ccreatedby) != trim($this->session->username) 
					&& trim($datas->app_t_usulan_iunorid) != trim($this->session->username)) */
				( !in_array(trim($datas->app_t_usulan_ccreatedby), $kode_lama) 
					&& !in_array(trim($datas->app_t_usulan_iunorid), $kode_lama) )
			) {
					unset($buttons['ubah']);
	...
	public function updateBox_app_t_usulan_lampiran($name, $value, $datas) {
		...
		//if ( (int)$datas->app_t_usulan_nsize2 > 0 ) {

			$input = "<input type='file' name='{$name}' id='{$name}' class='form-control {$name}' accept='application/pdf'/>";
			$input .= $this->session->sysparam->info_max_upload[0];
			if ( (int)$datas->app_t_usulan_nsize > 0 ){
				$input .= "<br/><span id='lbl_surat_usulan' data-toggle='modal' data-target='#myPreview_{$datas->app_t_usulan_id}' style='cursor:pointer;' class='label label-primary' 
							onclick='get_attachment({$datas->app_t_usulan_id}, \"myPreview_{$datas->app_t_usulan_id}\");'>
							<b>Surat Usulan</b>
						</span>";
			}
  ...
  function updateCheck_app_t_usulan_ijnsprubhnid($value, $post, $id) {
		  ...
		  $sql = "SELECT count(*) as total from app_t_usulan a
			...
			if ($total_request > 0) {
    			$sql = "SELECT count(*) as total from app_t_usulan a, 
    					...
    					a.iunorid = {$value} and b.istatus2 in (0,2) 
	...
	function updateCheck_app_t_usulan_iunorid($value, $post, $id) {
		...
		if ($this->input->post('app_t_usulan_istatus') == 1) {
			...
			if ( (int)$post->app_t_usulan_nsize > 0 ) {
				if ($this->gettotaldaftarpegawai($id) == 0) {	
					$data['status']  = false;
					$data['msg'] = 'Lengkapi daftar pegawai yang diusulkan dengan meng-klik tombol Tambah Pegawai';
					$data['obj'] = '#t_usulan_daftar #btn_new';
					$data['detil'] = true;
				} else {
	...
	function app_t_usulan_output() {
		$js = "<script type='text/javascript'>
				...
				$(document).ready(function() {
					...
					var status_usulan = $('#app_t_usulan_istatus').val();
					/* if( status_usulan == 0 ){
				...
				function get_attachment(id, divid) {
					var data = getHTML3('".base_url()."perbend/t_usulan_satker/get_attachment/'+id);
	...
	function generate($id) {
		...
		$sql = "SELECT 
						b.cnip as nip_new,
						b.ijabid2 as ijabid2,
						c.vname as nama_new,
						c.cgelardepan as gelardepan_new,
						c.cgelarbelakang as gelarbelakang_new,
						(select concat(pangkat, ', ', nama) 
						from kepeg_m_golongan where id = b.cgolid) as pangkat_new,
						(select nama from kepeg_m_jabatan where id = b.ijabid) as jabatan_new,
						(select vname from app_m_jabatan where id = b.ijabid2) as jabatan2_new, 
						(select cnobnt from kepeg_m_pegawai where cnip = b.cnip) as bnt_new,
						b.cnipold as nip_old,
						(select vname from kepeg_m_pegawai where cnip = b.cnipold) as nama_old,
						(select cgelardepan from kepeg_m_pegawai where cnip = b.cnipold) as cgelardepan_old,
						(select cgelarbelakang from kepeg_m_pegawai where cnip = b.cnipold) ascgelarbelakang_old,
						(select concat(pangkat, ', ', nama) 
									from kepeg_m_golongan where id = 
						(select cgolid from kepeg_m_pegawai where cnip = b.cnipold)) as pangkat_old,
						(select nama from kepeg_m_jabatan where id = (select ijabid from kepeg_m_pegawai where cnip = b.cnipold)) as jabatan_old,
						(select vname from app_m_jabatan where id = (select ijabid2 from kepeg_m_pegawai where cnip = b.cnipold)) as jabatan2_old,
						(select cnobnt from kepeg_m_pegawai where cnip = b.cnipold) as bnt_old,
						b.isbrubah 
					FROM app_t_usulan_pegawai b, kepeg_m_pegawai c 
					where b.cnip = c.cnip and b.iusulanid = {$id} 
  ...
  function send_email($next=0, $tos='', $pesan='', $post, $isnotif=FALSE) {
	...
	function get_detail_surat_sinde() {
		...
		$config = array(
			...
			'api_key'	=> $this->config->item('x_api_key'),//"b9729656-be4d-5700-9193-dcd3dbe1d1f3",
            'api_name'	=> "X-API-KEY"
		);
    ...
    [0] => Array
		...
		*/
		foreach($dataRows as $dr) {
			...
			if ( empty($bnt_old) ) $bnt_old = $bnt_new;
			...
			if ( empty($bnt_new) ) $bnt_new = $bnt_new;
			...
			if ( $dr['jabid2'] != 1 ) {
				$table->addRow();
				$table->addCell(700, $cellVCentered)->addText('');
				$table->addCell(1600, $cellVCentered)->addText('No. BNT', null, $cellHLEFT);
				$table->addCell(250, $cellVCentered)->addText(':', null, $cellHLEFT);
				$table->addCell(2650, $cellVCentered)->addText($bnt_old, null, $cellHLEFT);

				$table->addCell(1600, $cellVCentered)->addText('No. BNT', null, $cellHLEFT);
				$table->addCell(250, $cellVCentered)->addText(':', null, $cellHLEFT);
				$table->addCell(2650, $cellVCentered)->addText($bnt_new, null, $cellHLEFT);
			}
        ...
        ** UNTUK WINDOWS 
		...
		return array('file1'=>$escaped, 'file2'=>$escaped2pdf, 'size'=>$filesize);
	...
	function manipulate_list_button($buttons) {
		if ( $this->session->settahun != date('Y') ) unset($buttons);
