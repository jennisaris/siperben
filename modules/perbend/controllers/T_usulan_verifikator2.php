...
class T_usulan_verifikator2 extends MX_Controller {
  var $prefix = 'app';
  ...
  var $ar_statusperubahan = array();
	...
	public function __construct() {
		parent::__construct();
		$controller = "perbend/t_usulan_verifikator2";
		$table  = $this->prefix."_t_usulan";

   		$this->_setTitle('Usulan Verifikator');
		...
		$this->_changeType($table, 'iunorid', 'combobox2', $ar_unor);*/
		...
		$this->_addQuery($table, 'app_t_usulan.istatus = 2', 'and', '', 'true');
	...
	public function manipulate_update_button($buttons, $datas) {
	  $btn_simpankirim = "<button type='submit' id='btn_send' class='btn btn-primary' style='{$style}' 
								onclick='tinyMCE.triggerSave(true,true);$(\"#t_usulan_verifikator2_form-edit #app_t_usulan_istatus\").val(1);'>
	...
	function manipulate_list_button($buttons) {
		$buttons['add'] = "<button type='button' onclick='save_verifikasi2(\"".base_url()."perbend/t_usulan_verifikator2\", \"t_usulan_verifikator2\")' class='btn btn-primary' name='save_checklist' id='save_checklist' class='form-control save_checklist'>
	...
	function save_verifikasi2() {
      			...
      			if ( !empty($check_usulan1_status[$k]) ) {
					...
					$post = [
      				...
      				if ( $check_usulan_jenis[$k] == 2 )	{
						...
						$status += 3;
						$query2[] = "update app_t_usulan_pegawai set istatus2 = 1
		...
		foreach($id_usulan2 as $k=>$v) {
			if ( !empty($check_usulan2_status[$k]) )  { 
				$status = $check_usulan2_status[$k];		
				$status += 3;		
				if ( $check_usulan_jenis[$k] == 2 )	{
					$query2[] = "update app_t_usulan_pegawai 
							set istatus2 = 2, valasan2='".$alasan_usulan2[$k]."' 
							where iusulanid = {$v}";
				} else {
					$query2[] = "update app_t_usulan_pegawai set istatus2 = 2, valasan2='".$alasan_usulan2[$k]."' 
									where iusulanid = {$v} and istatus2 = 0";
		...
		if ( $this->db->trans_status === FALSE ) $ok = 0;
  ...
  function app_t_usulan_output(){
	$js ="<script type='text/javascript'>
			  ...
			  function klik(dis, b) {
				...
				if ( b == 1 ) {
					...
					if ( $('.check_usulan1').eq(i).is(':checked') ) {
						$('.check_usulan1_status').eq(i).val(2);
					...
					if ( $('.check_usulan2').eq(i).is(':checked') ) {
						$('.check_usulan2_status').eq(i).val(2);
	  ...
	  function save_verifikasi2(url, table_id, default_txt_confirm='Simpan. Anda yakin?', _ismodal=false, _modals='form-modal', _islochref=false, _isneedrefresh=true, _isOldFashion=false, _msg='Simpan Persetujuan berhasil.') {
				...
				save_confirm(url+'/save_verifikasi2', formData, default_txt_confirm, table_id, _ismodal, function(output) {
					var o = jQuery.parseJSON(output);
					$('div').removeClass('has-error');
					if ( o.status ) {
						//bootbox.alert(_msg);
						bootbox_alert('', '', _msg, true);
						reload_grid('".base_url()."perbend/t_usulan_verifikator/lists', 't_usulan_approval');
