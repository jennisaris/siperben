...
class T_usulan_daftar extends MX_Controller {
	...
	public function __construct() {
		...
		$this->_addField($this->table, 'ijabid', 'Kode Jab.', false, true);
		...
		if ( trim($this->uri->segment(5)) != '' ) $this->_add2ListField($this->table, 'isbrubah');
		...
		$this->_addOrderBy($this->table, ['ijabid2'=>'asc']);
	...
	function listBox_ACTION($buttons, $datas) {
		...
		$buttons['hapus'] = "<span style='text-align: center;cursor:pointer;' 
						onclick=\"hapus('".base_url().$this->router->fetch_module()."/".$this->router->class."/delete/".$datas->app_t_usulan_pegawai_id."', 'Hapus Record Usulan Satker (Lainnya)', '".strtolower($this->router->class)."', '".base_url().$this->router->fetch_module()."/".$this->router->class."/lists/".$this->session->{$this->table.'_page'}."', 'kriteria');\">
		...
		if ($datas->app_t_usulan_itipe == 0 ) {
			...
			if ( $this->url_opener == 't_usulan_verifikator' ||  
					...
					if ( $datas->app_t_usulan_istatus != 2 ) {
						unset($buttons['ubah']);
						//if ( trim($this->session->username) != trim($this->session->sysparam->superuser[0])) unset($buttons['hapus']);
						unset($buttons['hapus']);
					}
				...
				if ( $datas->app_t_usulan_pegawai_istatus == 2 || $datas->app_t_usulan_pegawai_istatus2 == 2 ) {
					...
					if ( $datas->app_t_usulan_istatus == 5 ) {
						if ($this->url_opener != 't_usulan_satker2' && 
							$this->url_opener != 't_usulan_satker') {
							unset($buttons['ubah']);
							//if ( trim($this->session->username) != trim($this->session->sysparam->superuser[0])) 
							unset($buttons['hapus']);
						}
					}
				}
		...
		if ( $datas->app_t_usulan_ijnsprubhnid == 1 ) unset($buttons['hapus']);
	  ...
	  $js ="<script type='text/javascript'>
          ...
          /*var no_sertifikat = {
	       ...
	       $(document).ready(function() {
					...
					//alert($('#{$this->table}_ijnsprubhnid').val());
					//alert($('#app_t_usulan_ijnsprubhnid').val());
					...
					if ( $('#app_t_usulan_ijnsprubhnid').val() == 1 ) {
						//tetap
						$('#t_usulan_daftar #btn_new').hide();
					} else $('#t_usulan_daftar #btn_new').show();
					...
					$('#{$this->table}_cnip').typeahead({
						...
						updater: function (item) {
							...
							$('#app_t_usulan_pegawai_ijabid').val(item.jabid);
	...
	function getemployeeold() {
		...
		/*
		SELECT c.cnip AS nip, a.vname AS nama
		FROM kepeg_m_pegawai a LEFT JOIN app_t_usulan_pegawai c ON a.cnip = c.cnip 
		LEFT JOIN kepeg_m_unor d  ON d.kode = c.ckduker
		WHERE c.inoskid != 0 
		AND c.inoskid IS NOT NULL AND c.isnonaktif = 0 
		-- And ( ( select kode_satker from kepeg_m_unor where kode = c.ckduker limit 1) = '677619' or c.ckduker = '677619')
		AND c.ckduker = '677619' 
		AND c.ijabid2 = '1' 
		*/
		...
		if ( $filter != '' ) {
			...
			or c.ckduker = '{$filter}') "; */
			$add_filter = " LEFT JOIN kepeg_m_unor d  ON d.kode = c.ckduker ";
			$add_filter3 = " AND c.ckduker = '{$filter}'";
		}
