...
class Detail_laporan3 extends MX_Controller {
  var $prefix = 'app';
  var $table;
  ...
  var $kriteria = [];
	public function __construct() {
		parent::__construct();
		$controller = "perbend/detail_laporan3";
		$this->table  = $this->prefix."_t_usulan";

    $this->_setTitle('Laporan PPK Bersertifikat (Per Satuan Kerja)');
	...
	function lists($page_ke=0, $reports=false, $kodeatasan='') {
      ...
      if ($query) {
        ...
        $html .= "<td>".$tot_all_5."</td>"; 
        $html .= "<td>".$tot_all_all."</td>"; 
        $html .= "</tr>";
  		
  		$html .= "</table>";
  		$pagination = $this->_ajaxPagination(base_url()."perbend/detail_laporan3/lists", $this->kriteria, 'detail_laporan3');
  		if ($reports) {
    ...
    if (!$reports) {
  	  $btn_download = "<button class='btn btn-primary' type='button' name='btn_download_detail[]' id='btn_download_detail_{$unor_id}' onclick='download(\"".base_url()."perbend/detail_laporan3/_detail/{$kode}/{$orgs}/1\");'><i class='fas fa-download'></i> Download</button>";
  	  $detail .= "<tr>";
  ...
  function manipulate_list_button($buttons) {
    ...
    $input = "<div class='modal fade' id='myModal_browse' role='dialog' aria-labelledby='myModalLabel' data-backdrop='static' data-keyboard='false'>
		...
		$buttons['download'] = "<button class='btn btn-primary' type='button' name='btn_download' id='btn_download' onclick='download(\"".base_url()."perbend/detail_laporan3/lists/0/1/\"+$(\"#q_app_t_usulan_iunorid\").val());'><i class='fas fa-download'></i> Download</button>";
