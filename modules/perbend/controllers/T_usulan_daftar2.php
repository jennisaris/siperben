...
class T_usulan_daftar2 extends MX_Controller {
  	var $prefix = 'app';
  	var $table;
	...
	public function __construct() {
		parent::__construct();
		$controller = "perbend/t_usulan_daftar2";
		...
		//$this->_addField($this->table, 'cjabid', 'Kode Jabatan.', false, true);
		$this->_addField($this->table, 'ijabid', 'Kode Jabatan.', false, true);
	...
	function listBox_ACTION($buttons, $datas) {
		...
		$buttons['hapus'] = "<span style='text-align: center;cursor:pointer;' 
						onclick=\"hapus('".base_url().$this->router->fetch_module()."/".$this->router->class."/delete/".$datas->app_t_usulan_pegawai_id."', 'Hapus Record Usulan Satker', '".strtolower($this->router->class)."', '".base_url().$this->router->fetch_module()."/".$this->router->class."/lists/".$this->session->{$this->table.'_page'}."', 'kriteria');\">
          ...
          /*var no_sertifikat = {
	       ...
	       $(document).ready(function() {
					...
					if ($('#{$this->table}_ijnsprubhnid').val() != 1 && $('#{$this->table}_ijnsprubhnid').val() != 9) {
					   ...
					   var url_ = '".base_url()."perbend/t_usulan_daftar2/getemployeeold';
					...
					/*$('#{$this->table}_cnipold_txt').typeahead({
						source: function (query, result) {
							$.ajax({
								url: '".base_url()."perbend/t_usulan_daftar2/getemployee',
				...
				function ubahStatus(id, usulanid, status, event) {
				  //alert(status);
					...
					if ( jwb ) {
						if ( event != 2 ) {
							$.post('".base_url()."perbend/t_usulan_daftar2/ubahstatus/'+id+'/'+usulanid+'/'+status+'/'+event, {}, function(data) {
							...
							edit('".base_url()."perbend/t_alasan_penolakan/edit/'+id+'/'+url_opener_, 't_usulan_daftar2');	
						}
					}
				}
	      </script>";
	...
	public function manipulate_url_save($save) {
		...
		$save['method'] = "save('".base_url()."perbend/t_usulan_daftar2', 't_usulan_daftar2', 'Simpan pegawai yang diusulkan. Anda yakin?', true, 'form-modal', false, true, false)";
		return $save;
	}
