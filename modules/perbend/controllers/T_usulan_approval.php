...
class T_usulan_approval extends MX_Controller {
  ...
  var $ar_statusperubahan = array();
	...
	public function __construct() {
		...
		$this->_add2ListField($table, 'check, check2, ijns, iunorid, cnousul, dtglusul, istatusid, ijnsprubhnid, lampiran, tupdated, cupdatedby');
	...
	function insertBox_app_t_usulan_lampiran($name) {
	...
	function insertCheck_app_t_usulan_cnousul($value, $post) {
		...
		if ( !empty($value) ) {
			if ( $_FILES['app_t_usulan_lampiran']['error'] != 0 ) {
				if ( trim($_FILES['app_t_usulan_lampiran']['type']) != 'application/pdf' ) {
					$data['status']  = false;
					$data['msg'] = 'Lampiran wajib dalam format PDF';
					$data['obj'] = 'app_t_usulan_lampiran';
				}	else if ( $_FILES['app_t_usulan_lampiran']['size'] > (int)$this->session->sysparam->max_size_upload[0] ) {
					$data['status']  = false;
	...
	function updateCheck_app_t_usulan_cnousul($value, $post, $id) {
	  return $this->insertCheck_app_t_usulan_cnousul($value, $post);
	}
	...
	function viewBox_app_t_usulan_daftarnama($name, $value, $datas) {
		$html = "<div>
        ...
        $js ="<script type='text/javascript'>
          ...
          function save_persetujuan(url, table_id, default_txt_confirm='Simpan. Anda yakin?', _ismodal=false, _modals='form-modal', _islochref=false, _isneedrefresh=true, _isOldFashion=false, _msg='Simpan Persetujuan berhasil.') {
				...
				function get_attachment(id, divid) {
					var data = getHTML3('".base_url()."perbend/t_usulan_approval/get_attachment/'+id);
