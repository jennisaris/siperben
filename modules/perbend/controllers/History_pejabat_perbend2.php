...
class History_pejabat_perbend2 extends MX_Controller {
  var $prefix = 'app';
  var $table;
  ...
  var $kriteria = [];
	public function __construct() {
		parent::__construct();
		$controller = "perbend/history_pejabat_perbend2";
		$this->table  = $this->prefix."_t_usulan_pegawai";

    	$this->_setTitle('Pejabat Perbendaharaan Lainnya (Saat ini)');
		...
		$header_controller = array('header_controller' => 'perbend/t_usulan_satker2');
	...
	function lists($page_ke=0) {
		...
		$sql = "SELECT id, cnip, vname, vjabnm, 
					...
					AND `isnonaktif` = '0' AND `ijabid2` IN (4, 5, 6, 7)";
		...
		$query = $this->db->query($sql); 
  		...
  		$html .= "</table>";
  		$pagination = $this->_ajaxPagination(base_url()."perbend/history_pejabat_perbend2/lists", $this->kriteria, 'history_pejabat_perbend2', $offset, $this->limit);
