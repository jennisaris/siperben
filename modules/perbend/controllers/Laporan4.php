...
class Laporan4 extends MX_Controller {
  var $prefix = 'app';
  var $table;
	public function __construct() {
		parent::__construct();
		$controller = "perbend/laporan4";
		$this->table  = $this->prefix."_t_usulan";

    $this->_setTitle('Laporan PPABP Bersertifikat');
	...
	function lists($page_ke=0, $reports=false) {
      ...
      //$html .= "<th>Jumlah</th>";
        ...
        //exit;
  		  ...
  		  >> <span onclick='_browse(\"".base_url()."perbend/detail_laporan4/index/0\");
              $(\"#detail_laporan4 #q_app_t_usulan_iunorid\").val(\"{$kode->kode_satker}\");
                reload_grid(\"".base_url()."perbend/detail_laporan4/lists\", \"detail_laporan4\");' 
              data-toggle='modal' data-target='#myModal_browse' data-backdrop='static' data-keyboard='false'>Detail Satuan Kerja</span>
  		  </td>";
  		  $html .= "<td align='center'>".$totals['total_7']."</td>";
        ...
        */
  		...
  		$html .= "</table>";
  		$pagination = '';//$this->_ajaxPagination(base_url()."perbend/laporan4/lists", $this->kriteria, 't_terbit_sk');
  		if ($reports) {
    ...
    $detail .= "</tr>";
      ...
      foreach($cols as $c) { 
        ...
        $detail .= "</tr>";
	  /*if (!$reports) {
  	  $btn_download = "<button class='btn btn-primary' type='button' name='btn_download_detail[]' id='btn_download_detail_{$unor_id}' onclick='download(\"".base_url()."perbend/laporan4/_detail/{$kode}/{$orgs}/1\");'><i class='fas fa-download'></i> Download</button>";
  	  $detail .= "<tr>";
  ...
  function manipulate_list_button($buttons) {
    ...
    $input = "<div class='modal fade' id='myModal_browse' role='dialog' aria-labelledby='myModalLabel' data-backdrop='static' data-keyboard='false'>
		...
		$buttons['download'] = "<button class='btn btn-primary' type='button' name='btn_download' id='btn_download' onclick='download(\"".base_url()."perbend/laporan4/lists/0/1\");'><i class='fas fa-download'></i> Download</button>";
