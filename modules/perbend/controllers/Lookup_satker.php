...
class Lookup_satker extends MX_Controller {
    ...
    $this->_setTitle('Lookup Satker');
		...
		$this->_addField($this->table, 'id_atasan', 'Satker Induk', false, true);
		...
		if ( !empty(trim($this->input->post('q_kepeg_m_unor_kode_satker'))) ) {
			$sql = "SELECT id from kepeg_m_unor where kode_satker = '".$this->input->post('q_kepeg_m_unor_kode_satker')."'";
			$result = $this->db->query($sql)->result();
			foreach ($result as $r) {
				$this->ar_unitkerja[$r->id] = $r->id;
			}
		}
	...
	function get_unit_by_kodesatker($b) {
		...
		$unors = array();
		$sql = "SELECT id from kepeg_m_unor where kode_satker = '".$b."'";
	...
	function listBox_action($buttons, $datas) {
		if ( in_array($datas->kepeg_m_unor_id, $this->ar_unitkerja) ) $checked = ' checked ';
		else $checked = ' ';
