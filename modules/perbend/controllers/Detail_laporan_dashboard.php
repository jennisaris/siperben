...
class Detail_laporan_dashboard extends MX_Controller {
  var $prefix = 'app';
  var $table;
  ...
  var $kriteria = [];
	public function __construct() {
		parent::__construct();
		$controller = "perbend/detail_laporan_dashboard";
	...
	function lists($page_ke=0, $reports=false, $kodeatasan='') {
      ...
      $html .= "<th>Jumlah</th>";
        ...
        */
  		...
  		$html .= "</table>";
  		$pagination = $this->_ajaxPagination(base_url()."perbend/detail_laporan_dashboard/lists", $this->kriteria, 'detail_laporan_dashboard');
  		if ($reports) {
    ...
    $detail .= "</tr>";
      ...
      foreach($cols as $c) { 
        ...
        $detail .= "</tr>";
	  /*if (!$reports) {
  	  $btn_download = "<button class='btn btn-primary' type='button' name='btn_download_detail[]' id='btn_download_detail_{$unor_id}' onclick='download(\"".base_url()."perbend/detail_laporan_dashboard/_detail/{$kode}/{$orgs}/1\");'><i class='fas fa-download'></i> Download</button>";
  	  $detail .= "<tr>";
  ...
  function manipulate_list_button($buttons) {
    ...
    $input = "<div class='modal fade' id='myModal_browse' role='dialog' aria-labelledby='myModalLabel' data-backdrop='static' data-keyboard='false'>
		...
		$buttons['download'] = "<button class='btn btn-primary' type='button' name='btn_download' id='btn_download' onclick='download(\"".base_url()."perbend/detail_laporan_dashboard/lists/0/1/\"+$(\"#q_app_t_usulan_iunorid\").val());'><i class='fas fa-download'></i> Download</button>";
