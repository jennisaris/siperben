...
class Rekap_pejabat2 extends MX_Controller {
  var $prefix = 'app';
  var $table;
	public function __construct() {
		parent::__construct();
		$controller = "perbend/rekap_pejabat2";
	...
	function lists($page_ke=0, $reports=false) {
  		...
  		$html .= "<table {$style} class='table table-bordered'>";
  		$html .= "<tr>";
  		$html .= "<th rowspan='2'>No.</th>";
  		$html .= "<th rowspan='2'>Eselon I</th>";
  		//$html .= "</tr><tr>";
  		$sql = "SELECT ckode FROM app_m_jabatan 
              WHERE id IN (2,3,6)";
      $cols = $this->db->query($sql)->result();
      foreach($cols as $c) {
        $html .= "<th colspan='2'>".$c->ckode."</th>"; 
      }
      $html .= "<th colspan='2'>Jumlah</th>";
      $html .= "</tr><tr>";
      for($i=0;$i<=sizeOf($cols);$i++) {
        foreach($sudah as $k=>$v) {
          $html .= "<th>".$v."</th>"; 
        }
      }
  		$html .= "</tr>";
        ...
        //exit;
  		  ...
  		  $html .= "<tr>";
  		  $html .= "<td>".$no."</td>";
  		  $html .= "<td><span onclick='$(\"#detail_{$kode->kode}\").toggle();'>".strtoupper($kode->nama)."</span>";
  		  ...
  		  //foreach($sudah as $k=>$v) {
  		    $x = 1;
  		    $html .= "<td style='text-align:center;'>".$totals['total_2'][$x]."</td>";
  		    $html .= "<td style='text-align:center;'>".$totals['total_2'][$x-1]."</td>";
  		    $html .= "<td style='text-align:center;'>".$totals['total_3'][$x]."</td>";
  		    $html .= "<td style='text-align:center;'>".$totals['total_3'][$x-1]."</td>";
  		    $html .= "<td style='text-align:center;'>".$totals['total_6'][$x]."</td>";
  		    $html .= "<td style='text-align:center;'>".$totals['total_6'][$x-1]."</td>";
  		  //}
  		  
  		  $total_1 = $totals['total_2'][$x] + $totals['total_3'][$x] + $totals['total_6'][$x];
  		  $total_0 = $totals['total_2'][$x-1] + $totals['total_3'][$x-1] + $totals['total_6'][$x-1];
  		  
  		  $html .= "<td style='text-align:center;'>".$total_1."</td>";
  		  $html .= "<td style='text-align:center;'>".$total_0."</td>";
  		  $html .= "</tr>";
        ...
        */
  		...
  		$html .= "</table>";
  		$pagination = '';//$this->_ajaxPagination(base_url()."perbend/rekap_pejabat2/lists", $this->kriteria, 't_terbit_sk');
  		if ($reports) {
    ...
    $detail .= "</tr>";
	  $sql = "Select x.id, x.ckode, x.vname, (select count(distinct concat(ckduker,'',cnip)) as total 
              ...
              (select count(distinct concat(ckduker,'',cnip)) as total 
              From app_t_usulan_pegawai a, 
              app_t_usulan b
              Where a.iusulanid = b.id 
              And a.ijabid2 = x.id and isnonaktif = 0 
      ...
      foreach($cols as $c) { 
        ...
        $detail .= "</tr>";
	  /*if (!$reports) {
  	  $btn_download = "<button class='btn btn-primary' type='button' name='btn_download_detail[]' id='btn_download_detail_{$unor_id}' onclick='download(\"".base_url()."perbend/rekap_pejabat2/_detail/{$kode}/{$orgs}/1\");'><i class='fas fa-download'></i> Download</button>";
  	  $detail .= "<tr>";
  ...
  private function get_total_perbendaharaan($ijabid2='', $kd_satker='') {
    //print_r($kd_satker);exit;
	  ...
	  $sql = "Select ijabid2, 0 as status, count(distinct concat(ckduker,'',cnip)) as total 
    ...
    $sql .= " UNION ";
    $sql .= "Select ijabid2, 1 as status, count(distinct concat(ckduker,'',cnip)) as total
              From app_t_usulan_pegawai 
              where {$q_ijabid2} ";
    if ($q_kd_satker !='') $sql.= $q_kd_satker;
	$sql .= " and case 
  ...
  function manipulate_list_button($buttons) {
    ...
    $input = "<div class='modal fade' id='myModal_browse' role='dialog' aria-labelledby='myModalLabel' data-backdrop='static' data-keyboard='false'>
		...
		$buttons['download'] = "<button class='btn btn-primary' type='button' name='btn_download' id='btn_download' onclick='download(\"".base_url()."perbend/rekap_pejabat2/lists/0/1\");'><i class='fas fa-download'></i> Download</button>";
