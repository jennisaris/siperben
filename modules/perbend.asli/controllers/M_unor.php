...
class M_unor extends MX_Controller {
  ...
  var $table;
    ...
    $this->_setTitle('Satuan Kerja');
		...
		$this->_addField($this->table, 'kode', 'Kode Satker', true, true);
		$this->_addField($this->table, 'kode_atasan', 'Satker Induk', true, true);
		$this->_addField($this->table, 'nama', 'Nama Satker', true, true);
		$this->_addField($this->table, 'deleted', 'Status Record', true, true);
  ...
  function save() {
    ...
    function getunor() {
    		...
    		$sql = "SELECT * 
    		    from app_m_unor a where (a.nama like '%".$kriteria."%'
