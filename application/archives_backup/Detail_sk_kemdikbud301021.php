	...
	function lists($page_ke=0, $reports=false, $kodeatasan='') {
      ...
      //print_r($_POST);
		...
		$sql = "select a.id, a.cnousul, a.dtglusul, a.iunorid,
				...
				app_t_usulan_pegawai b on a.id = b.iusulanid
				where a.istatus = '7' 
				and a.ijns = 1 ";// a.ctahun = '{$this->session->settahun}' and
		...
		$sql .= " order by a.ctahun, a.iunorid";
        ...
        //print_r($orgs);
			...
			if ( $offset == 0 ) $norut = $no;
			  ...
			  $html .= "<tr>";
			  $html .= "<td>".$norut."</td>";
			  $html .= "<td>".$kode->cnousul."</td>";
			  $html .= "<td>".($kode->dtglusul != null ? date('d-m-Y', strtotime($kode->dtglusul)) : '')."</td>";
			  $html .= "<td>".$kode->nama_unitutama."</td>";
			  $html .= "<td>".$kode->nama_satker."</td>";
			  $html .= "<td>".$kode->no_sk."</td>";
			  $html .= "<td>".($kode->tgl_sk != null ? date('d-m-Y', strtotime($kode->tgl_sk)) : '')."</td>";
			  $html .= "<td>".$kode->jenis_perubahan."</td>";
			  $html .= "<td>".$this->get_nama($kode->kpa, "ijabid2=1 and ckduker = {$kode->iunorid} and iusulanid != {$kode->id}")."</td>";
			   $html .= "<td>".$this->get_nama($kode->bp, "ijabid2=2 and ckduker = {$kode->iunorid} and iusulanid != {$kode->id}")."</td>";
			   $html .= "<td>".$this->get_nama($kode->bpn, "ijabid2=3 and ckduker = {$kode->iunorid} and iusulanid != {$kode->id}")."</td>";
